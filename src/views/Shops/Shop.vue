<template>
    <div class="shop" v-if="shopInfo">
        ...
        <div class="index-rst">
            <div class="rst-name">
                <span @click="isShow= true">{{shopInfo.rst.name}}</span>
                <i class="fa fa-caret-right"></i>
            </div>
             <!-- 弹窗信息 -->
            <InfoModel :rst="shopInfo.rst" :isShow="isShow" @close="isShow= false" />
            <!-- 评分月售 -->
            <div class="rst-order">
            ...
            <!-- 优惠信息 -->
            <Activity @choice="choiceActivity" 
            @open="isFlag=true" @close="isFlag=false" :isFlag="isFlag"
...
export default {
    ...
    data() {
        return {
            shopInfo: null,
            isShow: false,//是否显示商家信息弹窗
            isFlag: false//是否显示优惠弹窗
        }
    },
    components: {
        InfoModel,
    ...
    methods: {
        async getShopInfo(){
            const {data:res} = await this.$axios.get("/api/profile/batch_shop");
            // console.log(res);
            this.shopInfo=res;
        },
        choiceActivity(index){
            this.shopInfo.rst.activities[0]=this.shopInfo.rst.activities[index];
            this.isFlag= false;
        }
    }
}
